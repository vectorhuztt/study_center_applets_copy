...
class BasePage(object):
    ...
    @classmethod
    def set_device_name(cls, device_name):
    ...
    def wait_check_reload_page(self):
    ...
    def get_page_wait_ele(self, locator, timeout=10):
        """页面检查点问题"""
        try:
    ...
    def get_wait_check_page_ele(self, locator, single=True, applets=True, timeout=10):
        """
            页面检查点判断方法
            :param locator 定位方法
        ...
        try:
            if single:
                ele = WebDriverWait(self.driver, timeout, 0.5).until(lambda x: x.find_element(*locator))
            else:
                ele = WebDriverWait(self.driver, timeout, 0.5).until(lambda x: x.find_elements(*locator))
            return ele
